...
def _is_noise_image(img: Tag, src: str) -> bool:
    if not isinstance(img, Tag) or img.attrs is None:
        return True  # Treat as noise/unusable if it's malformed or not a standard Tag
    class_attr = img.get('class', '')
    if not isinstance(class_attr, str):
        class_attr = " ".join(class_attr)
    combined = f"{src} {class_attr} {img.get('alt', '')}".lower()
