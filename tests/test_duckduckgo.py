...
class TestResultFilter:
    ...
    def test_allow_social_flag(self):
        results = [self._make_result("https://reddit.com/r/news", snippet="A" * 50)]
        # By default, social is blocked
        assert filter_results(results) == []
        # When allow_social=True, social is allowed
        filtered = filter_results(results, allow_social=True)
        assert len(filtered) == 1
        assert filtered[0].url == "https://reddit.com/r/news"
