...
class Orchestrator:
    ...
    def run(
        ...
        *,
        ...
        if use_duckduckgo and keywords:
            self._collect_from_duckduckgo(keywords, max_results=ddg_max_results)
        ...
        # 4. Drain collector and run pipeline
    ...
    def _collect_from_duckduckgo(
        self, keywords: list[str], max_results: Optional[int] = None
    ) -> None:
        if max_results:
            self._ddg.max_results = max_results

        for kw in keywords:
            resp = self._ddg.search(kw)
            ...
            logger.info("DDG %r → %d new URLs", kw, n)
